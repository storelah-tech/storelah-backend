...
(function () {
  ...
  // ---------- floor-plan editor (facility setup view) ----------
  // Geometry is in LOGICAL GRID UNITS; the canvas renders grid->px at a zoom
  // scale factor. Placements persist on drop/move/resize-end via PUT
  // /floor-plans/:floorId/units/:unitId; canvas width/height/structure persist
  // via POST /floor-plans/:floorId on "Save Canvas".
  ...
  // Default on-canvas footprint (grid units) for a unit, derived from its size.
  ...
  // Grid cell under a client-space point, or null when outside the canvas.
  ...
  // Persist a placement geometry via PUT; reloads fresh state afterwards.
  ...
  // Grid-snapped drop of a palette unit onto the canvas. The plan is auto-created
  // server-side on first placement (lazy-init at the default canvas), so we adopt
  // the placed unit locally without a full reload (keeps zoom/selection stable).
  ...
  // Palette chip drag: a fixed-position ghost follows the pointer; on pointerup
  // the ghost's top-left lands on the snapped grid cell.
  ...
  // Drag an already-placed unit (or its resize handle) on the canvas.
  ...
  // Resize a placed unit from its bottom-right corner; grid-snapped, min 1×1.
  ...
  function switchView(view) {
    state.view = view;
    ['dashboard', 'units', 'tenants', 'floorplans'].forEach((v) => {
      const el = $(`#view-${v}`);
